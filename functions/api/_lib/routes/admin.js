...
export function applyAdminRoutes(app) {
  ...
  app.post('/admin/import', async (c) => {
    ...
    if (merchants) { for (const m of merchants) {
      await db.prepare(`INSERT OR REPLACE INTO merchants (id, email, password_hash, salt, provider_id, status, api_token_hash, api_token_prefix, api_token_created_at, last_sync_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(
        m.id, m.email, m.password_hash, m.salt, m.provider_id, m.status, m.api_token_hash || '', m.api_token_prefix || '', m.api_token_created_at || '', m.last_sync_at || '', m.created_at, m.updated_at
      ).run();
    } }
