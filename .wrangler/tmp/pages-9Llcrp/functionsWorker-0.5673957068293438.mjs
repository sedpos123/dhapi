...
__name(parseProvider, "parseProvider");
...
async function verifyJWT(token, secret) {
  ...
  if (payload.exp && payload.exp < Math.floor(Date.now() / 1e3)) throw new Error("Token expired");
  return payload;
}
...
app.put("/admin/brands/:id", async (c) => {
  ...
  for (const p of results) {
    ...
    if (idx >= 0) {
      arr[idx] = name;
      await db.prepare("UPDATE providers SET brands = ? WHERE id = ?").bind(JSON.stringify(arr), p.id).run();
    }
  }
  return c.json({ success: true });
});
app.delete("/admin/brands/:id", async (c) => {
  ...
  for (const p of results) {
    ...
    if (filtered.length !== arr.length) {
      await db.prepare("UPDATE providers SET brands = ? WHERE id = ?").bind(JSON.stringify(filtered), p.id).run();
    }
  }
  return c.json({ success: true });
});
...
app.post("/admin/import", async (c) => {
  ...
  if (brands) {
    for (const b of brands) {
      await db.prepare("INSERT OR REPLACE INTO brands (id, name) VALUES (?, ?)").bind(b.id, b.name).run();
    }
  }
  if (categories) {
    for (const cat of categories) {
      await db.prepare("INSERT OR REPLACE INTO categories (id, name) VALUES (?, ?)").bind(cat.id, cat.name).run();
    }
  }
  if (pending) {
...
var onRequest = /* @__PURE__ */ __name((context) => app.fetch(context.request, context.env, context), "onRequest");

// ../.wrangler/tmp/pages-9Llcrp/functionsRoutes-0.21903524914736716.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// C:/Users/16858/AppData/Roaming/npm/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// C:/Users/16858/AppData/Roaming/npm/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
...
// C:/Users/16858/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/16858/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// ../.wrangler/tmp/bundle-Y5UyVs/middleware-insertion-facade.js
...
// C:/Users/16858/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// ../.wrangler/tmp/bundle-Y5UyVs/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5673957068293438.mjs.map
